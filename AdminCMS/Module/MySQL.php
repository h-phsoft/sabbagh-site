...
function &ph_Connect($info = NULL) {
  ...
  $conn->Execute("SET time_zone = '" . PH_MYSQL_TIMEZONE . "'");
  return $conn;
}
...
if (!function_exists('ph_GetData')) {

  function ph_GetData($SQL) {
    $data = false;
    global $conn;
    if (!isset($conn)) {
      $conn = ph_Connect();
...
if (!function_exists('ph_Execute')) {
  ...
  function ph_Execute($SQL, $aValus = false, $callBack = NULL) {
    global $conn;
    if (!isset($conn)) {
      $conn = ph_Connect();
    }
    $rs = $conn->Execute($SQL, $aValus);
...
if (!function_exists('ph_ExecuteInsert')) {
  ...
  function ph_ExecuteInsert($SQL, $aValus = false) {
    global $conn;
    $ret = -1;
    if (!isset($conn)) {
      $conn = ph_Connect();
...
if (!function_exists('ph_ExecuteUpdate')) {
  ...
  function ph_ExecuteUpdate($SQL, $aValus = false) {
    global $conn;
    $ret = -1;
    if (!isset($conn)) {
      $conn = ph_Connect();
