...
class cEcomCat {
  ...
  public static function getSelectStatement($vWhere = '', $vOrder = '', $vLimit = '') {
    $sSQL = 'SELECT `id`, `status_id`, `order`, `wdays`, `name1`, `name2`, `image`, `descrip`'
      . ' FROM `ecom_cat`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `ecom_cat`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getFields($res) {
    ...
    $cClass->Description = $res->fields('descrip');
    $cClass->Image = $res->fields('image');
    if ($cClass->Image == null || $cClass->Image == 'null') {
      $cClass->Image = 'cat-0.png';
    }
    //
  ...
  public function save() {
    ...
    if ($this->Id == 0 || $this->Id == -999) {
      $vSQL = 'INSERT INTO `ecom_cat` ('
        . '  `status_id`, `order`, `wdays`, `name1`, `name2`, `descrip`, `image`'
        . ') VALUES ('
        . '  "' . $this->StatusId . '"'
        . ', "' . $this->Order . '"'
        . ', "' . $this->WDays . '"'
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `ecom_cat` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
