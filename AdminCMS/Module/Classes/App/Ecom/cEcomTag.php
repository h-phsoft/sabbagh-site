...
class cEcomTag {
  ...
  public static function getSelectStatement($vWhere = '', $vOrder = '', $vLimit = '') {
    $sSQL = 'SELECT `id`, `status_id`, `name`, `classname`'
      . ' FROM `ecom_tag`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `ecom_tag`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getFields($res) {
    ...
    $cClass->Classname = $res->fields('classname');
    //
  ...
  public function save($nUId) {
    ...
    if ($this->Id == 0 || $this->Id == -999) {
      $vSQL = 'INSERT INTO `ecom_tag` ('
        . '  `status_id`, `name`, `classname`, `ins_user`'
        . ') VALUES ('
        . '  "' . $this->StatusId . '"'
        . ', "' . $this->Name . '"'
        . ', "' . $this->Classname . '"'
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `ecom_tag` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
