...
class cEcomCurn {
  ...
  public static function getSelectStatement($vWhere = '', $vOrder = '', $vLimit = '') {
    $sSQL = 'SELECT `id`, `status_id`, `name`, `rate`, `color`, `symbole`'
      . ' FROM `ecom_curn`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `ecom_curn`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getFields($res) {
    ...
    $cClass->Symbole = $res->fields('symbole');
    //
  ...
  public function save($nUId) {
    ...
    if ($this->Id == 0 || $this->Id == -999) {
      $vSQL = 'INSERT INTO `ecom_curn` ('
        . '  `status_id`, `name`, `rate`, `color`, `symbole`, `ins_user`'
        . ') VALUES ('
        . '  "' . $this->StatusId . '"'
        . ', "' . $this->Name . '"'
        . ', "' . $this->Rate . '"'
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `ecom_curn` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
