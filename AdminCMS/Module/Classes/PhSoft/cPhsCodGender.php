...
class cPhsCodGender {
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `phs_cod_gender`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public function save($nUId) {
    ...
    if ($this->Id == 0 || $this->Id == -999) {
      $vSQL = 'INSERT INTO `phs_cod_gender` ('
        . '  `name`, `rem`, `ins_user`'
        . ') VALUES ('
        . '  "' . $this->Name . '"'
        . ', "' . $this->Rem . '"'
      ...
      $nId = $this->Id;
      $vSQL = 'UPDATE `phs_cod_gender` SET'
        . '  `name`="' . $this->Name . '"'
        . ', `rem`="' . $this->Rem . '"'
        . ', `upd_user`="' . $nUId . '"'
        . ' WHERE `id`="' . $this->Id . '"';
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `phs_cod_gender` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
