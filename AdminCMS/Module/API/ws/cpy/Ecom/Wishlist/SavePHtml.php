      ...
      if ($nSavedId > 0) {
        ...
        if (is_array($aRows) && count($aRows) > 0) {
          for ($ii = 0; $ii < count($aRows); $ii++) {
            $row = $aRows[$ii];
            $oTInstance = cEcomWishlist::getInstance(intval($row['nId']));
            $oTInstance->MstId = $nSavedId;
            if ($row['isDeleted'] === true || $row['isDeleted'] === 'true') {
              $oTInstance->delete();
            } else {
              $oTInstance->Id = intval($row['nId']);
              $oTInstance->Token = $row['vToken'];
              $oTInstance->Addat = $row['dAddat'];
              $oTInstance->StatusId = intval($row['nStatusId']);
              $oTInstance->ProdId = intval($row['nProdId']);
