...
if (isset($oRest)) {
  ...
  if (($nId == 0 && $oUser->oGrp->getPermission($oRest->getParameter('progId'))->Insert) ||
    ...
    $oInstance = cDistCountry::getInstance($nId);
    ...
    if ($oRest->getParameter('vFile')) {
      if ($oRest->getParameter('nId') > 0 && $oInstance->Image) {
        try {
          if (file_exists($vAttacheRootPath . 'Country/' . $oInstance->Image)) {
            unlink($vAttacheRootPath . 'Country/' . $oInstance->Image);
          }
        } catch (Exception $exc) {
        }
      }
      ...
      $oInstance->Image = base64_to_file($oRest->getParameter('vFile'), 'Country_' . $vFName, $oRest->getParameter('vFExt'), $vAttacheRootPath . 'Country');
    }
    try {
