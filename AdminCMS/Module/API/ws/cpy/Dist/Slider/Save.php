...
if (isset($oRest)) {
  ...
  if (($nId == 0 && $oUser->oGrp->getPermission($oRest->getParameter('progId'))->Insert) ||
    ...
    $oInstance = cDistSlider::getInstance($nId);
    ...
    if ($oRest->getParameter('vFile')) {
      if ($oRest->getParameter('nId') > 0 && $oInstance->Image) {
        try {
          if (file_exists($vAttacheRootPath . 'Slider/' . $oInstance->Image)) {
            unlink($vAttacheRootPath . 'Slider/' . $oInstance->Image);
          }
        } catch (Exception $exc) {
        }
      }
      ...
      $oInstance->Image = base64_to_file($oRest->getParameter('vFile'), 'Slider_' . $vFName, $oRest->getParameter('vFExt'), $vAttacheRootPath . 'Slider');
    }
    try {
