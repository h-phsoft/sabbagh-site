...
if (isset($oRest)) {
  ...
  if (($nId == 0 && $oUser->oGrp->getPermission($oRest->getParameter('progId'))->Insert) ||
    ...
    $oInstance = cDistVtypes::getInstance($nId);
    if ($oRest->getParameter('vFile')) {
      if ($oRest->getParameter('nId') > 0 && $oInstance->Image) {
        try {
          if (file_exists($vAttacheRootPath . 'Vtypes/' . $oInstance->Image)) {
            unlink($vAttacheRootPath . 'Vtypes/' . $oInstance->Image);
          }
        } catch (Exception $exc) {
        }
      }
      ...
      $oInstance->Image = base64_to_file($oRest->getParameter('vFile'), 'Vtypes_' . $vFName, $oRest->getParameter('vFExt'), $vAttacheRootPath . 'Vtypes');
    }
    try {
