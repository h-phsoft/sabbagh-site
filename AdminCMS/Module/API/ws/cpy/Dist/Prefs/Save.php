...
if (isset($oRest)) {
  ...
  if (($nId == 0 && $oUser->oGrp->getPermission($oRest->getParameter('progId'))->Insert) ||
    ...
    $oInstance = cDistPrefs::getInstance($nId);
    $oInstance->Id = $oRest->getParameter('nId');
    $oInstance->Name = $oRest->getParameter('vName');
    $oInstance->Val = $oRest->getParameter('vVal');
    if ($oRest->getParameter('vFile')) {
      if ($oRest->getParameter('nId') > 0 && $oInstance->Image) {
        try {
          if (file_exists($vAttacheRootPath . 'Prefs/' . $oInstance->Image)) {
            unlink($vAttacheRootPath . 'Prefs/' . $oInstance->Image);
          }
        } catch (Exception $exc) {
        }
      }
      ...
      $oInstance->Image = base64_to_file($oRest->getParameter('vFile'), 'Prefs_' . $vFName, $oRest->getParameter('vFExt'), $vAttacheRootPath . 'Prefs');
    }
    try {
