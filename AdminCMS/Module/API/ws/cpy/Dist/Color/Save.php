...
if (isset($oRest)) {
  ...
  if (($nId == 0 && $oUser->oGrp->getPermission($oRest->getParameter('progId'))->Insert) ||
    ...
    $oInstance = cDistColor::getInstance($nId);
    ...
    if ($oRest->getParameter('vFile')) {
      if ($oRest->getParameter('nId') > 0 && $oInstance->Image) {
        try {
          if (file_exists($vAttacheRootPath . 'Color/' . $oInstance->Image)) {
            unlink($vAttacheRootPath . 'Color/' . $oInstance->Image);
          }
        } catch (Exception $exc) {
        }
      }
      ...
      $oInstance->Image = base64_to_file($oRest->getParameter('vFile'), 'Color_' . $vFName, $oRest->getParameter('vFExt'), $vAttacheRootPath . 'Color');
    }
    try {
