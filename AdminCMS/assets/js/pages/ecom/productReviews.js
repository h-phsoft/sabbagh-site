  ...
  if (PhSettings.current.delete) {
    if ($('#fldId').val() > 0) {
      ...
      PhUtility.doDelete(vName, {
        'vOperation': 'cpy-Ecom-ProdReview-Delete',
        'nId': $('#fldId').val()
      }, openNew);
    }
  }
...
function save() {
  ...
  if (form.checkValidity()) {
    PhUtility.doSave({
      'vOperation': 'cpy-Ecom-ProdReview-Save',
      'nId': $('#fldId').val(),
...
function cellEditClick(e, cell) {
  ...
  $('#fldAddat').val(data.dAddat);
  $('#fldName').val(data.vName);
  $('#fldEmail').val(data.vEmail);
  $('#fldText').val(data.vText);
  toggleButtons();
  $('#ph_Modal').modal('show');
}
...
function refreshList() {
  ...
  table = getAjaxTabulator('cpy-Ecom-ProdReview-List', aColumns);
}
