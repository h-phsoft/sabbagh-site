...
function doEdit(nIdx) {
  ...
  $('#fldClassname').val(item.vClassname);
  $('#ph_Modal').modal('show');
}
      ...
      `<div class="col-sm-3 p-2 mx-auto">
        <div id="item-${item.nId}" class="card card-custom result-card h-100">
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-12">
                <h5>${item.vClassname}</h5>
                <h6>${item.vStatusName}</h6>
              </div>
            </div>
          </div>
  ...
  <table class="table table-striped table-bordered">
    <thead>
      <tr>
        <td></td>
        <td>${getLabel("Name")}</td>
        <td>${getLabel("Class")}</td>
        <td>${getLabel("Status")}</td>
        <td></td>
      </tr>
    <thead>
    <tbody>`;
