...
jQuery(document).ready(function () {
  ...
  mettaData.URLS = {
    "Save": {
      "URL": PhSettings.serviceURL + "/Team",
      "Method": "POST"
    },
    "Get": {
      "URL": PhSettings.serviceURL + "/Team",
  ...
  mettaData.ImagePath = PhSettings.mediaPath + 'Team/';
  ...
  if (isValidForm('ph_Form')) {
    $.ajax({
      ...
      data: {
        ...
        "vName": $('#fldName').val(),
        "vWork": $('#fldWork').val(),
        "vFacebook": $('#fldFacebook').val(),
        "vTwitter": $('#fldTwitter').val(),
        "vInstagram": $('#fldInstagram').val(),
        "vLinkedin": $('#fldLinkedin').val(),
        "vImage": $('#fldImage').val(),
        "vFExt": $('#fldFExt').val(),
        "vFName": $('#fldFName').val(),
        "vFile": $('#fldAttach').val()
...
function doEdit(nIdx) {
  ...
  $('#fldWork').val(item.vWork);
  $('#fldFacebook').val(item.vFacebook);
  $('#fldTwitter').val(item.vTwitter);
  $('#fldInstagram').val(item.vInstagram);
  $('#fldLinkedin').val(item.vLinkedin);
          ...
          <div class="card-body p-0">
            ...
            <div class="row pt-3 px-3">
              <div class="col-12">
                <div class="row">
                  <div class="col-12">
                    <span>${item.vImage}</span>
                  </div>
                  <div class="col-12">
                    <span>${item.vName}</span>
                  </div>
                  <div class="col-12">
                    <span>${item.vWork}</span>
                  </div>
                  <div class="col-12">
                    <span>${item.vFacebook}</span>
                  </div>
                  <div class="col-12">
                    <span>${item.vTwitter}</span>
                  </div>
                  <div class="col-12">
                    <span>${item.vInstagram}</span>
                  </div>
                  <div class="col-12">
                    <span>${item.vLinkedin}</span>
                  </div>
                </div>
              </div>
            </div>
